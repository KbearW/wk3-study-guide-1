...
def duplicates(items):
    ...
    #         if items[i] not in dupes_list:
    #             dupes_list.append(items[i])

    # Or

    for index, item in enumerate (items):
        if items[index-1] == items[index]:
            if items[index-1] not in dupes_list:
                dupes_list.append(items[index])
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    # in each word, if word[index] == letter
    # append the index number to the answer
    # if it doesn't exist in the list, append 'None'
    list1 = []
